	...
	public void move(SpaceShip theShip, ArrayList<PhotonTorpedo> torpedos){
		...
		for(Rock rock : rocks){
			...
			if(rock != this && rockPosition.intersects(rockToCheck)){
				...
				if(rockPosition.intersects(shipBox)){
				...
				for(PhotonTorpedo torpedo : torpedos){
					...
					if(torpedo.onScreen){
						...
						if(rockPosition.contains(torpedo.getXCenter(),torpedo.getYCenter())){
